...
const headerNumbers = [1,2,3,4,5,6] as const;

const mdxProps: MDXProps = {
	components: {
		...Object.fromEntries(headerNumbers.map(n => [
			`h${n}`,
			({ children }) => <Typography variant={`h${n}` as `h${typeof headerNumbers[number]}`} fontFamily="monospace" sx={theme => ({
	...
	return <Container>
		...
		<Stack component="article" sx={{
			...
			<Stack p={4} gap={1} sx={{ position: "relative" }}>
				...
				{ props.type === "link" && <>
					<Heading textTransform="none" component="p" sx={theme => ({
						mt: 2,
				...
				<Typography variant="h2" component="h1" fontFamily="monospace" textAlign="center" textTransform="uppercase" sx={{
					display: "flex",
				...
				{ props.type === "link" && post.description && (
					<Typography variant="body1" fontFamily="monospace" whiteSpace="pre-line" m={0} sx={theme => ({
